...
def connect_database():
    ...
    cur.execute(
        "create table(acc_no int, name text, age int, address text, balance int, account_type text, mobile_number int)"
    )
    cur.execute(
        "create table(name text, pass text,salary int, position text)"
    )
    cur.execute("create table admin (name text, pass text)")
    cur.execute("insert into admin values('memory','123')")
    conn.commit()
